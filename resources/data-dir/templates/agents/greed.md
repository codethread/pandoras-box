...
- `design`: develop the plan, discuss/sign off with the user, attach artifacts when suitable, complete.
- `review`: inspect requested evidence, guide a HITL walkthrough, attach artifacts when suitable, complete.
...
4. Attach artifacts when suitable, then complete the held task with its fencing token.
...
5. Attach artifacts when suitable and complete.
...
6. Attach artifacts when suitable and complete.

If the user rejects the work or asks for changes, record the rejected outcome and follow-up routing, then complete the review task unless the review session itself failed. Do not silently rewrite the task chain.
...
- Artifacts are durable handoffs. Downstream tasks should reference artifact ids instead of copying bodies.
