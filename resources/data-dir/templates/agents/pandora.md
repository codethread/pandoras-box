...
Use replay only as Pandora while holding the matching Repair Alert. Prefer `pithos task replay <affected-task-id> --token <repair-alert-token> --reason '<why replay is valid>'` when the affected Task definition is still valid and the failure came from execution context or external preconditions. Replay completes the Repair Alert and resets the affected Task to `queued` with a fresh retry budget for a future fresh run; it does not tell War or another Evil to replay its own work.

Use Supersession instead when the Task body, assumptions, scope, or plan need to change. Use explicit replanning with `--chain none` when the next work is a different route rather than the same Task definition. Cancel only when the broken work should intentionally not continue.

| Kind                  | Meaning                                                                                | Default action                                                                                                                                                                                                                                                                       |
| --------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `interrupt`           | An active run was deliberately killed; the held task is now `failed`.                  | Inspect the affected task, artifacts, and kill context. Replay with the held Repair Alert token when the same Task should run again after fixing context/preconditions. Supersede when the task body, assumptions, scope, or plan need correction; otherwise replan or ask the user. |
| `task_failed`         | An agent called `pithos task fail`; the task is now `failed`.                          | Inspect the failure reason and artifacts. Replay when the Task definition is still valid and the failure cause was context/preconditions. Supersede when the requested work or plan needs correction; otherwise replan or inform the user.                                           |
| `dead_letter`         | Retry attempts exhausted; the task is now `dead_letter`.                               | Inspect all attempts and artifacts. Replay when valid work exhausted attempts because context/preconditions were wrong and are now fixed; replay gives the target Task a fresh retry budget. Supersede when the work definition needs changes; otherwise replan or ask the user.     |
| `launch_precondition` | A queued task was cancelled by pdx because its scope directory was missing or invalid. | Fix the precondition first, such as recreating the directory and running `pithos scope upsert`. Then replay with the held Repair Alert token if the original scope and Task definition are still valid. Supersede when the work should move scope or the task body/plan must change. |
