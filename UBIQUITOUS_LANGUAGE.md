...
| **repair edge**           | A non-blocking branch-membership edge for a system-authored Repair Alert about broken work. It is repaired by Task Replay for valid same-Task retries, or by Supersession, replanning, or cancellation when the work definition changes or should not continue.                        | repair_source, normal context edge                                         |
...
| **Gate release**          | The per-Claim-sequence audit snapshot recording that a gated Task started after its target branch closure was clear.                                                                                                                                                                   | approval, permanent unblock                                                |
...
| **Task Replay**           | A Pandora-held Repair Alert resolution that resets the same failed, cancelled, or dead-lettered Task to queued work with a fresh retry budget while preserving audit history. Use Supersession instead when the Task definition must change.                                           | Retry if it hides Repair Alert authorization, resume, rerun                |
...
| **Repair Alert**          | A system-authored Alert with a typed `kind` indicating the cause of broken/blocked work. Kinds: `interrupt`, `task_failed`, `dead_letter`, `launch_precondition`, `reconciler_stuck`, `kill_failure`. Always authored by `run_pdx_system`; affected-task alerts carry a `repair` edge. | interruption escalation, launch-precondition escalation, system escalation |
...
| **Attempt**        | A resettable retry-cycle count used with `max_attempts` for dead-letter budgeting.                      | Lifetime claim id, failure count  |
| **Claim sequence** | A monotonic lifetime count of successful Claims for one Task; used as durable per-Claim audit identity. | Attempt, retry count, claim id    |
...
| **Task Replay** | The Pithos task transition that lets Pandora reset a matching failed/cancelled/dead-lettered Repair Alert target to queued same-Task work.                                    | Supersession, retry, resume      |
...
- A **Repair Alert** uses `repair`; **Pandora** should repair it with **Task Replay** for valid same-Task retries, or with **Supersession**, explicit replanning, or intentional cancellation when the work definition changes or should not continue.
...
- If **pdx** finds a **Claimable task** whose repo/worktree runtime path no longer exists before run creation, no **Run** is created. If that path disappears after a run row is created but before launch succeeds, **pdx** terminalizes the no-claim **Run** before repairing the queued task. In both cases **pdx** uses **Cancel** on the queued task and creates a **Repair Alert** for **Pandora** to repair with **Task Replay** after fixing the precondition, or with **Supersession** or replanning when the Task definition must change.
