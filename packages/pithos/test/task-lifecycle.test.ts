	...
	it("renders dependency forks in readable graph view", () => {
		...
		expect(graphText).toBe(
			[
				...
				"  preview: triage root",
				"  artifacts: none",
				`  - after ← ${design} [design] [blocked] design middle`,
				`    scope: ${repo}`,
				"    preview: design middle",
				"    artifacts: none",
				`    - after ← ${executeA} [execute] [blocked] execute fork A`,
				`      scope: ${repo}`,
				"      preview: execute fork A",
				"      artifacts: none",
				`      - after ← ${followUp} [execute] [blocked] execute follow-up`,
				`        scope: ${repo}`,
				"        preview: execute follow-up",
				"        artifacts: none",
				`    - after ← ${executeB} [execute] [blocked] execute fork B`,
				`      scope: ${repo}`,
	...
	it("deduplicates shared nodes in readable graph view for fan-in (diamond) dependencies", () => {
		...
		expect(graphText).toBe(
			[
				...
				"  preview: root task",
				"  artifacts: none",
				`  - after ← ${branchAlpha} [execute] [blocked] alpha branch`,
				`    scope: ${repo}`,
				"    preview: alpha branch",
				"    artifacts: none",
				`    - after ← ${shared} [execute] [blocked] shared leaf`,
				`      scope: ${repo}`,
				"      preview: shared leaf",
				"      artifacts: none",
				`  - after ← ${branchBeta} [execute] [blocked] beta branch`,
				`    scope: ${repo}`,
				"    preview: beta branch",
				"    artifacts: none",
