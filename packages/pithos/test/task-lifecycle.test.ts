...
`);
	...
	it("rejects legacy artifacts tables instead of migrating alpha data", () => {
		const { dbPath, engine } = setup();
		const db = new Database(dbPath);
...
`);
		...
		expect(() => engine.init({ fresh: false })).toThrow(
			/artifacts table schema is incompatible.*pithos init --fresh/s,
		);
	});

	it("enforces active/rejected artifact status constraints on current schema", () => {
		const { dbPath, engine } = setup();
		const taskId = engine.enqueue({
			scope: "global",
			capability: "triage",
...
`);
	...
	it("renders dependency forks in readable graph view", () => {
		...
		expect(graphText).toBe(
			[
				...
				`  scope: ${repo}`,
				"  preview: triage root",
				`  - after ← ${design} [design] [queued] design middle`,
				`    scope: ${repo}`,
				"    preview: design middle",
				`    - after ← ${executeA} [execute] [queued] execute fork A`,
				`      scope: ${repo}`,
				"      preview: execute fork A",
				`      - after ← ${followUp} [execute] [queued] execute follow-up`,
				`        scope: ${repo}`,
				"        preview: execute follow-up",
				`    - after ← ${executeB} [execute] [queued] execute fork B`,
				`      scope: ${repo}`,
				"      preview: execute fork B",
				"",
			].join("\n"),
		);
	});
	...
	it("includes preview lines and artifact refs in graph node output", () => {
		const { engine, repo } = setup();
		const taskId = engine.enqueue({
			scope: repo,
			capability: "execute",
			title: "preview task",
			body: "# Heading\n\nTicket: https://example.test/T-1\nNext line",
			bodyFile: undefined,
			runId: "run_toil",
			after: [],
			chain: "none",
		...
		const graph = engine.graphInspect({ taskId, scope: undefined, all: false });
		expect(graph.graph.nodes).toEqual([
			expect.objectContaining({
				id: taskId,
				preview: "preview task",
				artifact_refs: [{ id: artifactId, kind: "note", title: "evidence" }],
			}),
		]);
	});
	...
	it("deduplicates shared nodes in readable graph view for fan-in (diamond) dependencies", () => {
		...
		expect(graphText).toBe(
			[
				...
				`  scope: ${repo}`,
				"  preview: root task",
				`  - after ← ${branchAlpha} [execute] [queued] alpha branch`,
				`    scope: ${repo}`,
				"    preview: alpha branch",
				`    - after ← ${shared} [execute] [queued] shared leaf`,
				`      scope: ${repo}`,
				"      preview: shared leaf",
				`  - after ← ${branchBeta} [execute] [queued] beta branch`,
				`    scope: ${repo}`,
				"    preview: beta branch",
	...
	it("deduplicates successor nodes in readable graph view when they appear in both supersession and dependency positions", () => {
		...
		expect(graphText).toBe(
			[
				...
				`  scope: ${repo}`,
				"  run: run_war",
				"  preview: alpha parent",
				`  - after ← ${original} [execute] [failed] alpha original`,
				`    scope: ${repo}`,
				"    run: run_war",
				"    preview: alpha original",
				`    - repair ← ${repairAlert} [escalate] [queued] Investigate failed task ${original}`,
				"      scope: global",
				`      preview: Investigate failed task ${original}`,
				`    ↻ replaced-by ${successor} [execute] [queued] beta successor`,
				`      scope: ${repo}`,
				"      preview: beta successor",
