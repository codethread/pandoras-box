...
import { parseHarnessKind, requireNonEmpty } from "./validation.js";
...
const heldTask = (db: Db, taskId: string): TaskRow =>
	decodeRow(
		TaskRowSchema,
	...
	runCleanup: ({ runId, reason, sessionEvidence }) =>
		withDb(ctx, (db) => {
			...
			const finalRun: RunRow = db.transaction((): RunRow => {
				...
				const task = heldTask(db, run.task_id);
