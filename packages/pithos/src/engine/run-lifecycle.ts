...
import { enforceActiveScope, runById, runSelect, scopeById } from "./actors.js";
...
import type { Engine, EngineContext, RunOutput } from "./types.js";
import { parseHarnessKind, requireNonEmpty } from "./validate.js";
...
const heldTaskRow = (db: Db, taskId: string): TaskRow =>
	decodeRow(
		TaskRowSchema,
	...
	runCleanup: ({ runId, reason, sessionEvidence }) =>
		withDb(ctx, (db) => {
			...
			const finalRun: RunRow = db.transaction((): RunRow => {
				...
				const task = heldTaskRow(db, run.task_id);
