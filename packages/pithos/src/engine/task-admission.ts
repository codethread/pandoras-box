...
import { requireNonEmpty, resolveBody, resolveRunId } from "./inputs.js";
import { enforceReleasedGateLateGrowth } from "./late-growth.js";
import { authorized, liveRun } from "./run-read-model.js";
import { enforceTaskAdmissionScope } from "./scopes.js";
import {
	branchClosure,
	canonicalTaskId,
	insertTaskEdge,
...
const assertAcyclic = (
	db: Db,
	includeEdge: (kind: string) => boolean,
	cycleMessage: string,
): void => {
	const outgoing = new Map<string, string[]>();
	for (const edge of taskEdges(db).filter((row) => includeEdge(row.kind))) {
	...
	const visit = (id: string): void => {
		if (visiting.has(id)) fail("VALIDATION_ERROR", cycleMessage);
...
const assertGraphIntegrity = (db: Db): void => {
	assertAcyclic(db, (kind) => kind !== "gate", "task branch-membership cycle detected");
	assertGateOwnersOutsideTargetClosures(db);
	assertAcyclic(db, (kind) => kind === "after" || kind === "gate", "task blocking cycle detected");
};

export const makeTaskAdmissionOps = (
	ctx: EngineContext,
