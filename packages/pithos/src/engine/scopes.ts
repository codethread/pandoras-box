...
import { decodeRow, ScopeRowSchema, type ScopeRow } from "../rows.js";
import { withDb } from "./db-helpers.js";
import { requireNonEmpty } from "./inputs.js";
import type { Engine, EngineContext, ScopeIdentityOutput, ScopeOutput } from "./types.js";
...
const scopePathMissing = (
	ctx: EngineContext,
	scope: { readonly kind: ScopeRow["kind"]; readonly canonical_path: string | null },
): boolean =>
	...
	scopeList: ({ all }) =>
		withDb(ctx, (db) => ({
			...
			scopes: db
				...
				.map((scope) => ({
					...scope,
					path_missing: scopePathMissing(ctx, scope),
				})),
		})),
				...
				(): {
					...
					return {
						...
						scope: {
							...
							path_missing: scopePathMissing(ctx, archivedScope),
						},
					};
				},
			)(),
