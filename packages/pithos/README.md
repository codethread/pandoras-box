...
- Context commands (`task inspect`, `graph inspect`, `briefing`) render readable text by default and expose `--json` for structured output. `task inspect` readable output is a single-task dossier: full task body, compact active artifact refs, direct local context only, and an associated run id when known from active ownership or task lifecycle evidence; pass `--full` to render active artifact bodies inline. `graph inspect` readable output is a threaded map of compact task cards for the same unpruned graph selection returned by `--json`: topology, typed edges, title-based preview lines, associated run ids when known, non-empty active artifact refs, gate branch members, provenance, and Supersession history. Agenda/sitrep summaries stay with `briefing`.
