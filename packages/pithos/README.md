...
`src/engine.ts` is the composition root for the Pithos domain API used by both the CLI and `pdx`: `makeEngine` assembles the per-concern op factories below plus `init`, and the file re-exports the public Engine surface (types, renderers, shared read helpers). The behavior lives in the engine submodules:

- `src/engine/types.ts` — public Engine input/output contracts.
- `src/engine/render.ts` — pure task/graph/briefing text rendering.
- `src/engine/validation.ts` — input-shaping helpers shared by transitions (`requireNonEmpty`, run-id/body resolution, harness-kind parsing).
...
- `src/engine/run-read-model.ts` — run row reading, live-run/actor capability authorization, and run/task status terminality helpers.
- `src/engine/scopes.ts` — scope upsert/list/archive plus scope-capability admission policy, including repo/worktree directory admission checks.
- `src/engine/run-lifecycle.ts` — Run upsert/inspect/Cleanup/Interrupt/timeout/launch-abort transitions, including dead-letter Repair Alert evidence rendering.
- `src/engine/task-authoring.ts` — task enqueue/supersede/replay transactions, including active-scope and repo/worktree directory admission checks.
...
- `src/engine/task-views.ts` — read-only projections: task inspect, graph inspect, briefing, and artifact list/show.
...
- `src/engine/graph-integrity.ts` — whole-graph invariants asserted inside write transactions: blocking/membership acyclicity and gate-owner closure exclusion.
