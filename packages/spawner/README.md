...
- HITL mode launches under tmux. `fagent` HITL launches `exec` the fake Harness directly so later newline-delimited input reaches the original process; if startup exits non-zero, the shell exits loudly.
