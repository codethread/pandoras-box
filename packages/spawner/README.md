...
- HITL mode launches under tmux. `fagent` HITL launches `exec` the fake Harness directly so the original process remains the pane target for later newline-delimited input; if startup exits non-zero, the shell exits loudly.
