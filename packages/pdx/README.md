...
Captures process boundary inputs, builds the Effect service layer, dispatches parsed CLI operations to controller operations, and prints tagged JSON errors on stderr.

### `src/cli.ts` — CLI surface

Defines the `pdx` command tree with `@effect/cli`, including human help, machine-readable help JSON, command argument/option descriptions, and the internal `daemon run` startup path.
...
- When a ready repo/worktree task's cwd is missing before run creation, pdx uses Pithos' atomic launch-precondition transition to cancel the still-queued task, create a global Repair Alert (kind=`launch_precondition`) with a `repair` edge for Pandora, and avoid creating a Run. When `<user-data-dir>/supervisor.toml` enables `launch_preconditions.enforce_repo_root_trunk`, repo-scoped ready tasks also pass a local-only Git default-branch probe before render or Run creation; non-Git paths, unknown `origin/HEAD`, detached HEAD, and non-default current branches use the same Repair Alert transition with branch evidence and Task Replay guidance. Global and worktree scopes are exempt. If the cwd disappears after run creation but before launch succeeds, pdx first calls Pithos' launch-abort transition so the no-claim Run becomes `cancelled` with reason `launch_precondition_failed`, then applies the same atomic task transition.
