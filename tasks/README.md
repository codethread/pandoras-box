...
- Task 21: Extended `@pdx/fagent` HITL mode so `hitl: true` startup scripts emit `FAGENT_HITL_READY`, log `hitl_startup`, and process later newline-delimited stdin inputs through the same config and original process environment. CLI-driven script events now include `instance_id` and `process_id`; focused tests prove a later `repair` action shares both with startup. Deep-review follow-up removed Spawner's `tail -f /dev/null` fagent HITL wrapper so the original fagent process remains the tmux pane target, tightened `hitl`/`executeScopeId` config validation, and added resident stdin failure coverage. YAGNI pass removed a redundant residency-only CLI test and replaced timer polling with stream-output waits in the remaining HITL tests. The Podman integration startup config was switched to explicit resident HITL mode to keep current verification green, but replacing its `tmux respawn-pane` repair shortcut with stdin input remains Task 22 scope. Verified with focused `pnpm --filter @pdx/fagent test`, focused `pnpm --filter @pdx/spawner test`, and full `pnpm verify`.
- Task 22: Updated the Podman `pdx open` fagent integration so the pdx-launched Pandora config includes both startup and later `repair` scripts, then sends `repair` to the existing `pdx--pandora` tmux session with `tmux send-keys` instead of replacing the pane command. The integration now asserts Pandora repair events share the startup `run_id`, `instance_id`, and `process_id`, and that the tmux pane pid is unchanged across repair input. Verified with focused `pnpm run test:integration:pdx-open-fagent` and full `pnpm verify`.
- Task 23: Aligned the fagent, pdx, Spawner, root integration, and supervision/Harness docs with the Task 22 no-respawn repair flow. The docs now state that fagent HITL emits `FAGENT_HITL_READY`, processes later line-oriented stdin commands in the original pdx-launched process, fails loudly on missing/bad input or script errors, and leaves JSONL event evidence proving stable run/process identity. The Podman `pdx open` documentation now explicitly rejects `tmux respawn-pane` as an MVP repair shortcut. Validation run: `pnpm verify`.
